...
- отрядное исследование доведено до общего маршрута: после победы CTA ведёт в `⚔️ Исследовать вместе`, лидер может получить не только бой, но и общую exploration-сцену для всего отряда, а бездействующий участник в бою после таймаута пропускается автоатакой;
- вторая earnable-валюта `✨ Сияние` без новых игровых таблиц: `Player.radiance` хранится в Prisma, показывается в главном меню/профиле/инвентаре, начисляется мягким `Следом дня`, проходит через `ResourceReward`, reward ledger, telemetry `resourceRadianceDelta` / `rewardRadianceDelta` и release evidence economy report;

- economy balance pass для пыли/осколков/эссенций без новых таблиц: крафт и перековка рун теперь тратят рунную пыль поверх существующих осколков, UNUSUAL+ крафт требует ранние материалы вроде эссенции, rune hub показывает стоимость, а тесты моделируют 30-60 минут ранних боёв на избыток ресурсов;
- rune action hub приведён к одному player-facing названию `🕯 Алтарь`: главная кнопка, экран действий, recovery-copy и resource-find тексты больше не смешивают алтарь с отдельной мастерской;
- slice алхимии пилюль переехал в расходники Мастерской: добыча тратится на четыре пилюли восстановления здоровья, маны и краткой боевой защиты, навык `Алхимия` повышает выход, а применение работает в бою и между встречами без постоянного роста статов;

- anti-stall recovery rails без новых таблиц: низкое HP ведёт в `🌿 Передышку` с восстановлением HP/маны через `PlayerProgress`, серия поражений переключает next-goal на руны/осторожный путь, а encounter resolver временно выбирает лёгкую обычную встречу без элит, боссов и roaming-скачков;
- encounter variety v1 поверх существующих локаций и врагов: встреча теперь может прийти как `засада`, `свежий след`, `усталый враг` или `элитный след`, меняя инициативу, шанс отступления и стартовое HP без новых биомов/таблиц; standalone exploration pool получил безопасную находку;
- First-hour difficulty guardrails для раннего маршрута: adaptive difficulty теперь снижает следующую угрозу при низких HP/мане, случайные elite-встречи не появляются до раннего safe-порога, верхний roaming выключен до уровня угрозы `10`, а поражение восстанавливает игрока до безопасного floor по HP/мане без новых таблиц и отдельной rest-системы;
- post-defeat flow доведён до player-facing маршрута: результат поражения показывает, что добыча не начислена, что руны/пыль/материалы/школа сохранены, сколько HP/маны восстановлено, и даёт путь `🔮 Руны -> ⚔️ Осторожно дальше`; следующий explore при `defeatStreak > 0` не форсит school elite/miniboss и не тянет higher-biome roaming;
- combat choice v1 углубляет enemy intent без новых ресурсов: тяжёлый удар телеграфируется раньше и делает `защиту` сильнее, guard-break чаще требует атаки или готовой руны, активная руна получает небольшой бонус по раскрытому замыслу врага, а battle clarity/keyboards прямо показывают выбор между атакой, защитой и руной;
- school combat identity pass v1 усиливает 4 школы через текущие руны и intent-окна без новых таблиц: Пламя давит guard-break базовой атакой, Твердь лучше держит раскрытый тяжёлый удар, Буря получает tempo-guard через `Шаг шквала`, а Прорицание в result copy называет прочитанный intent;
- RARE seal progression доведён до следующего горизонта без новых школ/таблиц: экипированная печать даёт малый school-specific боевой payoff, открывает next-goal `Цель печати` и второй mastery-ранг школы;
- mastery milestones v1 добавляет мини-ветку из 4 ранних видимых вех школы на опыте `1/3/5/7`: прогресс открывается текущими победами с надетой школьной руной, а профиль и руны показывают открытые, ближайшую и закрытые вехи без новых таблиц и большого дерева навыков;
- optional `✨ След дня` добавлен как daily-free activity slice без streak/FOMO: один мягкий след календарного дня выдаёт малую награду через существующий reward ledger, повтор показывает non-punitive copy и telemetry `daily_trace_claimed` / `daily_trace_already_claimed`;
- readability pass активных рун в бою без изменения их силы: общий read-model доступности теперь объясняет на экране и кнопках, готова ли руна, сколько стоит маны, сколько осталось отката, почему маны не хватает или почему сейчас не тот момент;
- `📖 Бестиарий` усилен как механический помощник без новых таблиц: после первой встречи враг показывает повадку и рабочий ответ против своей угрозы, а добыча и шанс руны по-прежнему раскрываются только после первого `APPLIED` трофея;
- `📜 Книга путей` усилена как навигация без новых таблиц: страницы по 5 записей теперь сначала выводят готовые награды, затем один ближайший actionable след, остальные следы и компактный архив, сохраняя лимит VK-клавиатуры в 6 рядов;

- `📜 Книга путей` получила карусель страниц по 5 записей: записи идут по главам/школам, готовые квесты помечены `🎁 Награда не собрана`, а inline-кнопки claim показываются только для наград на текущей странице вместе с навигацией назад/вперёд;
...
- `release:local-playtest` и release evidence теперь закрывают полный ранний school loop по четырём школам: `novice elite -> UNUSUAL sign -> equip sign -> school miniboss -> RARE seal`;
- payload auto-equip школьного знака во второй свободный слот теперь даёт тот же compact payoff recap и telemetry `equip_school_sign`, что и явная установка в основной слот;
...
- RF-005 combat cleanup: `BattleEngine` reduced to a clone-and-delegate facade, while action dispatch, battle guards and encounter/flee resolution moved into focused domain components with encounter tests;
- RF-004 combat cleanup: enemy basic attacks, prepared intents, guard-break/heavy-strike execution and signature reaction outcomes moved from `BattleEngine` into focused `battle-enemy-actions` with dedicated tests;
...
- `Книга путей` теперь рендерится как постраничная книга глав: вместо единого приоритетного полотна игрок видит до 5 записей на странице, сгруппированных по главам/школам, с явным маркером несобранной награды;
...
- `Книга путей` больше не падает на отправке VK-клавиатуры, когда готовых наград больше пяти: inline-кнопки наград ограничены лимитом VK в 6 рядов вместе с кнопкой возврата в меню.
