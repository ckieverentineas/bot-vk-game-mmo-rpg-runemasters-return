...
Текущая очередь структурного наведения порядка живёт в [`docs/platform/component-refactor-roadmap.md`](docs/platform/component-refactor-roadmap.md). Она режет cleanup на маленькие запросы: один архитектурный шаг, одна проверка, один коммит. Это защищает уже работающие игровые фичи от большого рискованного рефакторинга.
