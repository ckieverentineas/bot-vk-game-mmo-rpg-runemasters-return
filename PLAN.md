...
- School-specific hidden trophy actions теперь покрывают все четыре стартовые школы на novice enemy-срезе: Пламя вытягивает знак из ash-seer, Твердь выбивает печать из stonehorn-ram, Буря перехватывает trace у storm-lynx, а Прорицание считывает omen у blind-augur; условие учитывает оба экипированных слота рун, а все действия идут через тот же pending reward snapshot и exact-once collect rail.
- Early game закрывает главный маршрут `исследовать -> событие/бой -> добыча -> следующий шаг`: next-step подсказки вынесены в отдельные `💡`-блоки, pending-добыча явно просит выбрать действие, а после победы всегда доступны понятные кнопки для добычи, рун, исследования и пати.
...
- Action-based trophy rewards имеют local handler manual evidence для pending trophy collect/replay в `docs/testing/pending-trophy-manual-playtest-q022.md`, но ещё не считаются полностью release-proven без release evidence или release-owner решения. Новые school hidden trophy actions по Тверди, Буре и Прорицанию закрыты доменными snapshot-тестами; живой VK smoke по ним остаётся желательным перед крупной выкладкой. Глубокие многоступенчатые threshold-лестницы и stat growth остаются будущими работами.
