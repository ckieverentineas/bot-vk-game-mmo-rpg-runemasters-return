...
- Межбоевой attrition включён для HP/маны: `PlayerProgress` хранит текущие vitals, следующий battle snapshot стартует с них, а поражение оставляет игроку безопасный минимум `1 HP`.
...
- `📜 Книга путей` имеет первый vertical slice постоянных квестов: записи идут каруселью по 5, группируются по главам/школам, готовые награды помечены `🎁 Награда не собрана`, а claim идёт через inline-кнопки exact-once поверх reward ledger.
