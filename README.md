...
- после установки первого знака школа теперь ещё и ведёт дальше одним явным действием `⚔️ Проверить школу`, чтобы игрок быстрее переходил из rune hub обратно в бой без menu wandering;
...
- `docs/reviews/support-rune-slot-v1.md` и `docs/reviews/support-rune-slot-v2.md` — исторические review-срезы старой модели второго слота; актуальный runtime перешёл на два равноправных стартовых слота;
...
Текущий срез — это **первый playable multi-rune combat slice**: в бою работают две стартовые руны, их полные статы, пассивы и активные кнопки. Passive-only руны остаются полезными через пассивы и статы, а мана понемногу возвращается при смене хода обратно к игроку.
...
- rune hub показывает карусель по 5 рун, статус `выбрана / надета N`, а действия `надеть в слот / снять / распылить` появляются только после выбора конкретной руны;
