...
- HP и мана теперь сохраняются между боями через `PlayerProgress`: следующая встреча стартует с состоянием после прошлой схватки, а поражение оставляет минимум `1 HP`;
- в exploration появились бродячие встречи: мобы из ближайшего старого биома иногда заходят выше по уровню, а мобы из ближайшего более опасного биома могут очень редко появиться снизу;
...
- `Книга путей` теперь открывает квестовые главы с плотным приоритетом: готовые награды, один ближайший след и компактно закрытые записи; claim идёт через exact-once `RewardLedgerRecord`, а legacy text `забрать награду` получает canonical replay receipt;
- post-battle trophy progression теперь поддерживает малые action-based срезы: базовый сбор добычи, threshold-действия для свежевания/реагентов/эссенции и enemy-kind действия для существующих трофейных врагов через тот же pending reward rail;
...
- если первый знак школы уже получен, но ещё не надет, school-aware next-goal теперь ведёт не в абстрактный гринд, а прямо в `🔮 Руны`, фокусирует этот знак и помогает закрепить признание школы в реальной сборке;
...
- `книга путей` / `квесты` / `задания` / `летопись пути` — открыть главы пути: готовые награды, ближайший незавершённый след и компактно закрытые записи;
...
Чтобы динамическая сложность не стирала старых врагов из игры, encounter resolver иногда подмешивает мобов из ближайшего нижнего биома (`12%`) и очень редко — из ближайшего верхнего биома (`3%`). Боссы так не кочуют.
