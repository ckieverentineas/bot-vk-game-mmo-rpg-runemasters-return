...
- `📜 Книга путей` с каруселью записей по 5: главы/школы группируют путь, готовые награды помечены `🎁 Награда не собрана`, claim остаётся exact-once через reward ledger;
...
- `книга путей` / `квесты` / `задания` / `летопись пути` — открыть главы пути каруселью по 5 записей с маркером `🎁 Награда не собрана`;
