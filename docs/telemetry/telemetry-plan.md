...
- [x] Onboarding / post-session / return / economy / exploit signals покрыты.
  - Runtime пишет все baseline signal families. Открытые caveats: return follow-up proxy пока 0 в release evidence, а economy source coverage пока стартует с `QUEST_REWARD` claims.
