...
| `school_novice_follow_up_action_taken` | игрок сделал первый осмысленный шаг после novice payoff | `school_code`, `current_goal_type`, `action_type`, `sign_equipped`, `used_school_sign`, `battle_id`, `enemy_code`, `event_version` | проверка, что первый school payoff не заканчивается dead end экраном |
