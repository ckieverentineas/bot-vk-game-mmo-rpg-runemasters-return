...
`resourceRadianceDelta` / `rewardRadianceDelta` добавлены к economy/daily trace payloads, чтобы вторая валюта `✨ Сияние` сразу попадала в release evidence и не растворялась в общей пыли.
...
| `return_recap_shown` | existing player получил return recap | `entrySurface`, `hasEquippedRune`, `currentSchoolCode`, `nextStepType`, `event_version` | clarity первого действия после return и качество школьной guidance |
...
| `daily_trace_claimed` | игрок впервые забрал optional след дня за календарный день | `playerId`, `activityCode`, `gameDay`, `claimedNow`, `rewardDustDelta`, `rewardShardsDelta`, `event_version` | мягкий activity evidence без streak/FOMO |
| `daily_trace_already_claimed` | игрок повторно открыл след дня, который уже был забран | `playerId`, `activityCode`, `gameDay`, `claimedNow`, `rewardDustDelta`, `rewardShardsDelta`, `event_version` | replay/UX guard без второй награды |
| `quest_book_opened` | игрок открыл `Книгу путей` | `playerId`, `questCode`, `questStatus`, `readyToClaimCount`, `claimedCount`, `event_version` | видимость quest book funnel в release evidence |
| `quest_reward_claimed` | quest reward применён впервые | `playerId`, `questCode`, `questStatus`, `readyToClaimCount`, `claimedCount`, `event_version` | проверка, что quest reward flow реально выдаёт награду |
| `quest_reward_replayed` | повтор старой quest reward кнопки обработан replay-safe | `playerId`, `questCode`, `questStatus`, `readyToClaimCount`, `claimedCount`, `event_version` | reward duplication guard для quest rewards |
| `quest_reward_not_ready` | игрок запросил quest reward, который ещё не готов | `playerId`, `questCode`, `questStatus`, `readyToClaimCount`, `claimedCount`, `event_version` | ранний сигнал UX confusion вокруг quest readiness |
...
- школьный `nextStepType` помогает быстрее дойти до ближайшего school payoff или заполнить свободный полноценный рунный слот.
...
- `economy_transaction_committed` для committed quest reward и daily trace claims (`transactionType = reward_claim`, `sourceType = QUEST_REWARD` / `DAILY_TRACE`);
- `daily_trace_claimed`;
- `daily_trace_already_claimed`;
- `quest_book_opened`;
...
Состояние на 2026-04-23: runtime instrumentation покрывает все families из telemetry v1 DoD. Остаточные `warn` в `release:evidence` остаются product-evidence gaps, а не отсутствующей instrumentation: `return_recap_shown` уже пишется, но пока без follow-up proxy; economy summary покрывает `QUEST_REWARD` и `DAILY_TRACE`; post-payoff loadout evidence всё ещё показывает, что novice sign не доводится до сборки во всех школах.
...
Current runtime coverage includes `QUEST_REWARD` and `DAILY_TRACE` claims; battle rewards and pending trophy rewards should reuse the same stable event shape when their economy source paths are added.
...
- [x] Каждая ключевая question above сопоставлена хотя бы с одним event.
  - Onboarding и first school payoff: `onboarding_started`, `tutorial_path_chosen`, `first_school_presented`, `first_school_committed`, `school_novice_elite_encounter_started`, `reward_claim_applied`.
  - Post-session и return clarity: `post_session_next_goal_shown`, `return_recap_shown`, `school_novice_follow_up_action_taken`, `loadout_changed`.
  - Loadout, economy и exploit health: `loadout_changed`, `economy_transaction_committed`, `reward_claim_applied`, `battle_stale_action_rejected`.
- [x] Event names и required fields описаны.
  - Event map v1 перечисляет shipped event names и payload fields; quest book reward events добавлены в этот snapshot.
- [x] Onboarding / post-session / return / economy / exploit signals покрыты.
  - Runtime пишет все baseline signal families. Открытые caveats: return follow-up proxy пока 0 в release evidence, а economy source coverage пока стартует с `QUEST_REWARD` и `DAILY_TRACE` claims.
- [x] Owner и weekly review cadence названы.
  - Product / UX, Balance / Gameplay и QA / Release owners перечислены в review rhythm.
- [x] Scope boundaries задокументированы, чтобы не разрастаться в analytics platform.
  - Out of scope и Instrumentation rules явно запрещают dashboard/warehouse/vendor creep, clickstream spam и full BI в telemetry v1.
