...
Healthy signal:
- заметная доля игроков с `hunt_school_elite` доходит до aligned novice elite, получает reward/payoff и затем делает loadout touch или следующий PvE шаг вместо menu wandering.
