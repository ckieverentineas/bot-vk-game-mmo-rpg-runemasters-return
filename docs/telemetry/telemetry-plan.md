...
|---|---|---|---|
| `onboarding_started` | новый игрок входит в первый flow | `entry_surface`, `event_version` | базовая точка funnels |
...
| `post_session_next_goal_shown` | battle result показал next-goal block | `battle_outcome`, `had_rune_drop`, `suggested_goal_type`, `event_version` | проверка качества post-session guidance |
| `return_recap_shown` | existing player получил return recap | `entry_surface`, `has_equipped_rune`, `current_school_code`, `next_step_type`, `event_version` | clarity первого действия после return |
| `loadout_changed` | equip / unequip / swap реально меняет текущую сборку | `change_type`, `before_school_code`, `after_school_code`, `before_rarity`, `after_rarity`, `event_version` | engagement с rune/loadout loop |
