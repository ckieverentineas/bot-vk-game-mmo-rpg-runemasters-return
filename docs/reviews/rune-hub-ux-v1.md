...
- rune hub может показывать ближайшую school-веху и её payoff, но не превращается в отдельное progression-меню.

## Why this slice

- убирает путаницу между `selected rune` и `equipped rune`;
- поддерживает быстрый выбор в `1–2` нажатия;
- даёт игроку ответ не только на вопрос `какая руна выбрана`, но и `зачем мне сейчас идти в бой или ставить поддержку`;
