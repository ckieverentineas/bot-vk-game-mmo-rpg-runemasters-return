  ...
  switch (battle.player.runeLoadout?.schoolCode) {
    ...
    case 'stone':
      if (hasEquippedSchoolSeal(battle, 'stone') && hasReadableIntentCode(battle, reading, 'HEAVY_STRIKE')) {
        return '🛡️ Печать Тверди: тяжёлый удар стал целью печати — держите стойку и превращайте угрозу в опору.';
      }

      if (hasEquippedSchoolSeal(battle, 'stone')) {
        return '🛡️ Печать Тверди: защита получает устойчивую опору; следующий горизонт — искать цели, которые проверяют выдержку.';
      }

      if (firstSchoolSignEquipped && hasReadableIntentCode(battle, reading, 'HEAVY_STRIKE')) {
        return '🛡️ Первый знак Тверди: держите тяжёлый удар защитой или «Каменным отпором», а затем отвечайте сильнее обычного.';
      }

      if (firstSchoolSignEquipped && hasReadableIntentCode(battle, reading, 'GUARD_BREAK')) {
        return '🛡️ Первый знак Тверди: пробивающий удар наказывает слепую стойку; добавьте ответный ход, а не только защиту.';
      }

      if (firstSchoolSignEquipped) {
        return '🛡️ Первый знак Тверди: школа уже в руках; переживите опасный ход и отвечайте сильнее, чем обычная руна.';
      }

      if (hasReadableIntentCode(battle, reading, 'HEAVY_STRIKE')) {
        return '🛡️ Твердь: тяжёлый удар лучше держать защитой или «Каменным отпором», а затем наказать в окно ответа.';
      }

      if (hasReadableIntentCode(battle, reading, 'GUARD_BREAK')) {
        return '🛡️ Твердь: пробивающий удар ломает чистую стойку; не ставьте весь ход только на защиту без ответа.';
      }

      return '🛡️ Твердь: ценность школы в том, чтобы пережить опасный ход и ответить сильнее обычного.';
    case 'gale': {
