...
const encounterHintByEnemyCode: Partial<Record<string, { schoolCode: string; genericHint: string; schoolHint: string }>> = {
  'ash-seer': {
    ...
    schoolHint: 'Подсказка: школа Пламени здесь особенно полезна — этого врага выгоднее дожимать и давить уроном, чем уходить в глухую защиту.',
  },
  'stonehorn-ram': {
    schoolCode: 'stone',
    genericHint: 'Подсказка: этот элитный враг позже готовит тяжёлый удар, так что защита и ответный ход здесь особенно ценны.',
    schoolHint: 'Подсказка: школа Тверди здесь особенно полезна — защита и каменный ответ помогают пережить его разгон и наказать в окно после удара.',
  },
};
